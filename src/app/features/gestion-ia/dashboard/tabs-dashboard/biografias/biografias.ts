...
@Component({
  ...
  templateUrl: './biografias.html',
  styleUrl: './biografias.scss',
})
export class Biografias implements OnInit {
  ...
  cargar(): void {
    ...
    this.error.set(null);
    this.gestionIaService.listar('biografia').subscribe({
      next: (presets) => {
        this.presets.set(presets);
        this.cargando.set(false);
      },
      error: () => {
        this.error.set('No se pudieron cargar los presets de biografía.');
        this.cargando.set(false);
      },
    });
  }
